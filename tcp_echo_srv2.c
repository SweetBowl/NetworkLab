...
//  main.c
//  tcp_echo_srv2
//  Created by 赵旭 on 2020/7/2.
//  Copyright © 2020 Zhaox. All rights reserved.
...
int echo_rep(int sockfd)
{
    ...
    do {
        ...
        // 的设定需要用到这两个变量
        int read_amount = 0, len_to_read = len_h;
        ...
        do{
            ...
            res = read(sockfd, buf+8+read_amount, len_to_read);
            ...
            read_amount += res;
            if (read_amount == len_h) {
                break;
            }
            else if (read_amount < len_h){
                len_to_read = len_h -read_amount;
            }
            else{
                free(buf);
                return pin_h;
